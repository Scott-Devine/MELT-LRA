...
function update_meis() {
    ...
    state.meis.forEach(mei => {
        ...
        // check for CALU/LINEU substitution *not* overlapping with a S-W alignment span
        let have_orphan_subs = false
        ...
        subs.forEach(sub => {
            ...
            if (!has_span_overlap) {
                have_orphan_subs = true
            }
        })
        
        if (has_del_overlap) {
            //            console.log("overlap in " + mei['chrom'] + ':' + mei['pos'])
            n_with_overlap += 1
        }
        if (have_orphan_subs) {
            //           console.log("orphan subs in " + mei['chrom'] + ':' + mei['pos'])
